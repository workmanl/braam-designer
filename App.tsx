...
import type { BraamSettings, LayerSettings, GlobalSettings, Preset, OscillatorType, NoiseType } from './types';
...
type Action =
  ...
  | { type: 'LOAD_SETTINGS'; payload: BraamSettings }
  | { type: 'RANDOMIZE' }
  | { type: 'RESET' };

const settingsReducer = (state: BraamSettings, action: Action): BraamSettings => {
  switch (action.type) {
    case 'UPDATE_LAYER':
      return {
        ...state,
...
function App() {
  const [storedSettings, setStoredSettings] = useLocalStorage<BraamSettings>('braam-settings', INITIAL_SETTINGS);
  ...
  const handleRender = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `braam_${timestamp}.wav`;
    ...
    <div className="min-h-screen bg-gray-950 text-gray-300 p-4 lg:p-8 flex flex-col gap-4">
      ...
      <header className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 bg-gray-900 rounded-lg shadow-lg">
        <h1 className="text-3xl font-bold text-white tracking-wider">
          BRAAM <span className="text-accent">DESIGNER</span>
        </h1>
